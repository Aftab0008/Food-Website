...
const dishes = [...food_items, ...food_items];
...
function About() {
  ...
  const scrollRight = () => {
    if (sliderRef.current) {
      ...
      setCurrentIndex((prevIndex) => (prevIndex + 1) % food_items.length); // Loops back
    }
  };
  ...
  const scrollLeft = () => {
    if (sliderRef.current) {
      sliderRef.current.scrollBy({ left: -300, behavior: "smooth" });
      setCurrentIndex((prevIndex) => (prevIndex - 1 + food_items.length) % food_items.length);
    }
  };
  // AutoPlay Effect
  useEffect(() => {
    const interval = setInterval(() => {
      if (currentIndex >= food_items.length - 1) {
        sliderRef.current.scrollTo({ left: 0, behavior: "smooth" }); // Reset to start
        setCurrentIndex(0);
      } else {
        scrollRight();
      }
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <section className="w-full  h-[60vh] md:h-[92vh] flex justify-center items-center bg-black relative mt-15">
  {/* Background Image */}
  <img 
    src="https://media-hosting.imagekit.io//4901e131d9f24db7/view-beautifully-decorated-round-table-with-natural-fir-branch-candle-two-flutes-plates-against-classic-sofa-modern-apartment.jpg?Expires=1835250639&Key-Pair-Id=K2ZIVPTIP2VGHC&Signature=UjGWhqFaWEPw9cLTgW0TraA3-a755AvzJUHgg2YGNl46U4HlQG06-7TD0kTjBfngPMo0UBi~09AS-ywscadb515ng5ZIwBzBcGEpsqW3u0YEqFgvsTGYRC2VdtGXwty5a7s2yN8aQ8lAUc2s5Jqs6B-ps53IiCYb-yRd4-1zs5DuPdLlGvPGBiGW4K02Bpyq7V4zy84fYXhNoK8gmEYLHF-aWVoLj1r9rvoadpgqthRZVeYadUifMK3ijLZ1ObMUcXI8LKPuIXxTMvgZWq0Bm2GmiY4siMpnZXF5vU-7mfWvM2-mX5MUz2UvHNIoBB2WKPEUsVTO4AzvOLkrRocgyw__" 
  ...
  <div className="absolute text-center text-white">
    <h1 className="text-3xl md:text-5xl font-bold">About Our Restaurant</h1>
    <p className="text-lg md:text-xl mt-2">Delicious food made with passion</p>
  </div>
</section>
...
<section className="py-16 px-5 md:px-20 text-center font-serif bg-sky-100">
  <div className="w-full h-auto flex flex-col items-center">
    ...
    <div className="mt-10 p-6  rounded-xl   max-w-5xl">
      <h3 className="text-3xl md:text-4xl font-semibold text-green-700 mb-10 underline-offset-auto">Our Signature Dishes</h3>
   </div>
 ...
 <section className="py-12 px-5 md:px-5 text-center bg-sky-300 flex flex-col justify-between items-center">
      <div className="w-full h-[60vh] md:h-[87vh] bg-sky-700 p-3 rounded shadow-md">
        <h2 className="text-3xl font-bold text-sky-500 mb-8">Our Special Dishes</h2>
        <div className="relative">
          <button
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-300 p-3 rounded-full shadow-md"
            onClick={scrollLeft}
          >
          ...
          <div ref={sliderRef} className="flex overflow-hidden space-x-5 px-10">
            {dishes.map((dish, index) => (
              <motion.div
                key={index}
                className="min-w-[300px] md:min-w-[350px] p-6 bg-white shadow-lg rounded-xl"
          ...
          <button
            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gray-300 p-3 rounded-full shadow-md"
            onClick={scrollRight}
          >
